    ...
    const char * const dst_chars) 
{
    ...
    STRUCTURED_DEBUG(debug_flag,printf("%s src(%s) strlen(%d) src_chars (%s) len(%d) dst_chars(%s)\n", csu, src, strlen(src), src_chars, strlen(src_chars), dst_chars));

    size_t len = strlen(src);
    for (int i = 0; i < len; i++) {
        char * pos = strchr(src_chars, src[i]);
        STRUCTURED_DEBUG(debug_flag,printf("%s %d %c\n", csu, i, *pos));
        if (!pos) {
    ...
    STRUCTURED_DEBUG(debug_flag,printf("%s dst(%s)\n", csu, dst));
    return 0;
}
    ...
    const uint32_t * const dst_chars) 
{
    ...
    STRUCTURED_DEBUG(debug_flag,printf("dst_chars (%S) len(%d)\n", dst_chars, u32_strlen(dst_chars)));
