...
int invoke(ubiq_platform_operation_context_t * ctx, char32_t ** const output) {
  ...
  if (ctx) {
    ...
    if (ubiq_platform_operation_context_get_is_encrypt(ctx)) {
      UBIQ_DEBUG(debug_flag, printf("ubiq_platform_operation_context_get_is_encrypt true\n"));
      CTX_CAPTURE_ERROR(ctx, res, "Decrypt operation is not allowed in a encryption pipeline ");
      return res;
    } 
    ...
    if (dataset) {
      ...
      UBIQ_DEBUG(debug_flag, printf("output_chars (%S)\n", output_chars));
      ...
      // int x = 0;
      // while (*ptr != U'\0') {
      //   const uint32_t * pos = u32_strchr(output_chars, *ptr);
      //   if (!pos) {
      //     UBIQ_DEBUG(debug_flag, printf("Input string has an invalid character (%C) x(%d)\n", *ptr, x));
      //     printf("x: %d\n", x);
      ...
      char * utf8_ct = NULL;
      res = convert_utf32_to_utf8(current_value, (uint8_t**)&utf8_ct);
      UBIQ_DEBUG(debug_flag, printf("utf8_ct (%s) res(%d)\n", utf8_ct, res));
      UBIQ_DEBUG(debug_flag, printf("tweak NULL?(%d)\n", tweak == NULL));
       UBIQ_DEBUG(debug_flag, printf("tweak_len(%d)\n",  tweak->len));
      char * utf8_pt = calloc(1, strlen(utf8_ct) * 3); // magic number to account for multi-byte
      res = ff1_decrypt(ff1_ctx, utf8_pt, utf8_ct, 
          tweak->buf, tweak->len);
      UBIQ_DEBUG(debug_flag, printf("ff1_decrypt utf8_pt(%s) res(%d) \n", utf8_pt, res));

      if (!res) {
        res = convert_utf8_to_utf32(utf8_pt, output);
      }
      free(utf8_ct);
      free(utf8_pt);
    } // dataset
...
ubiq_platform_operation_t * const ubiq_platform_decrypt_operation_create(void) {
  ubiq_platform_operation_t * s = calloc(1, sizeof(ubiq_platform_operation_t));
  s->invoke = &invoke;
  s->getType = &getType;
  s->ctx = s;
  s->destroy = &ubiq_platform_decrypt_operation_delete;
  return s;
}

void ubiq_platform_decrypt_operation_delete(ubiq_platform_operation_t * const op) {
  free(op);
}
