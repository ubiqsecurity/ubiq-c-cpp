  ...
  if (ctx) {
    ...
    if (dataset) {
      ...
      if (pad_char == '\0') {
        ...
        res = ubiq_platform_trim_left_pad(pad_char, current_value, &out);
        uint32_t const * const template = ubiq_platform_operation_context_get_data_value(ctx, OPERATION_CONTEXT_PASSTHROUGH_TEMPLATE);
        if (template != NULL) {
          uint32_t * trimmed_template = NULL;
          res = ubiq_platform_trim_left_pad(pad_char, template, &trimmed_template);
          if (!res) {
            res = ubiq_platform_operation_context_put_data_value(ctx, OPERATION_CONTEXT_PASSTHROUGH_TEMPLATE, trimmed_template);
            free(trimmed_template);
          }
        } else {
          res = 0;
        }
...
ubiq_platform_operation_t * const ubiq_platform_unpad_input_operation_create(void) {
  ...
  s->getType = &getType;
  s->ctx = s;
  s->destroy = &ubiq_platform_unpad_input_operation_delete;
  return s;
}
