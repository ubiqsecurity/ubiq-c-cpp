...
struct ubiq_platform_decryption_pipeline {
...
static int add_base_operations(ubiq_platform_structured_pipeline_t * const pipeline)
{
  ...
  if (!res) {res = ubiq_platform_structured_pipeline_add_operation(pipeline, 
    ubiq_platform_decode_key_number_operation_create(), -1);}

  if (!res) {res = ubiq_platform_structured_pipeline_add_operation(pipeline, 
    ubiq_platform_convert_radix_operation_create(), -1);}

  if (!res) {res = ubiq_platform_structured_pipeline_add_operation(pipeline, 
    ubiq_platform_decrypt_operation_create(), -1);}

  if (!res) {res = ubiq_platform_structured_pipeline_add_operation(pipeline, 
    ubiq_platform_unpad_input_operation_create(), -1);}

  if (!res) {res = ubiq_platform_structured_pipeline_add_operation(pipeline, 
    ubiq_platform_decode_input_operation_create(), -1);}
  ...
  ubiq_platform_structured_pipeline_t * const pipeline)
{
  ...
  for (int i = UBIQ_PASSTHROUGH_RULES_COUNT - 1; !res && i >= 0; i--) {
    UBIQ_DEBUG(debug_flag, printf("i(%d) rule_priorities(%d)\n", i, rule_priorities[i]));
...
{
  int res = -ENOMEM;
  ubiq_platform_decryption_pipeline_t * p = calloc(1, sizeof(ubiq_platform_decryption_pipeline_t));
  ...
  if (res) {
    ...
    ubiq_platform_decryption_pipeline_delete(p);
    p = NULL;
  }
