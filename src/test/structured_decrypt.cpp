...
class cpp_structured_decrypt : public ::testing::Test
{
public:
    void SetUp(void);
    void TearDown(void);
...
void cpp_structured_decrypt::SetUp(void)
{
    ASSERT_TRUE((bool)_creds);
}

void cpp_structured_decrypt::TearDown(void)
{
}

TEST_F(cpp_structured_decrypt, none)
{
    ASSERT_NO_THROW(
...
void
cpp_structured_decrypt::encrypt_decrypt(
    const std::string & ffsname,
    const std::string & pt)
{
    std::string ct, rec, ct2, pt2;
    ubiq::platform::structured::decryption dec;
    ubiq::platform::structured::encryption enc;
    std::vector<std::uint8_t> tweak;

    enc =  ubiq::platform::structured::encryption(_creds);
    dec =  ubiq::platform::structured::decryption(_creds);
...
TEST_F(cpp_structured_decrypt, normal)
{
...
TEST(c_structured_decrypt, piecewise_bad_char)
{
...
TEST(c_structured_decrypt, piecewise_too_short)
{
...
TEST(c_structured_decrypt, piecewise_all_zeroth)
{
...
TEST(c_structured_decrypt, piecewise_all_passthrough)
{
...
TEST(c_structured_decrypt, piecewise_leading_passthrough)
{
...
TEST(c_structured_decrypt, add_user_defined_metadata)
{
    ...
    struct ubiq_platform_credentials * creds;
    struct ubiq_platform_structured_enc_dec_obj *enc;
    struct ubiq_platform_structured_enc_dec_obj *dec;
    char * buf(nullptr);
    char * ctbuf(nullptr);
    size_t ctlen;
    size_t len;
    ...
    res = ubiq_platform_structured_enc_dec_create(creds, &dec);
    ASSERT_EQ(res, 0);
    ...
    res = ubiq_platform_structured_enc_dec_add_user_defined_metadata(dec, toolong);
    EXPECT_NE(res, 0);

    res = ubiq_platform_structured_enc_dec_add_user_defined_metadata(dec, "not json");
    EXPECT_NE(res, 0);

    res = ubiq_platform_structured_enc_dec_add_user_defined_metadata(dec, "{\"UBIQ_SPECIAL_USER_DEFINED_KEY\" : \"UBIQ_SPECIAL_USER_DEFINED_VALUE\"}");
    EXPECT_EQ(res, 0);
    ...
    res = ubiq_platform_structured_decrypt_data(dec,
      ffs_name, NULL, 0, ctbuf, ctlen, &buf, &len);
    EXPECT_EQ(res,0);
    free(buf);

    // decrypt will have meta data
    res = ubiq_platform_structured_enc_dec_get_copy_of_usage(dec, &buf, &len);
    EXPECT_EQ(res,0);
    ...
    ubiq_platform_structured_enc_dec_destroy(enc);
    ubiq_platform_structured_enc_dec_destroy(dec);
...
TEST_F(cpp_structured_decrypt, add_user_defined_metadata)
{
  std::string pt(";0123456-789ABCDEF|");
  std::string ffs_name("ALPHANUM_SSN");

  ubiq::platform::structured::encryption enc = ubiq::platform::structured::encryption(_creds);
  _dec = ubiq::platform::structured::decryption(_creds);
