...
static int create_operation_context(
  ...
  if (!res && suffix_length >= 0) {
    cJSON * p = cJSON_CreateObject();
    cJSON_AddStringToObject(p, "type", "suffix");
    cJSON_AddNumberToObject(p, "value", suffix_length);
...
TEST(operation_trim_passthrough_suffix, getType)
{
  ubiq_platform_operation_t * const t = ubiq_platform_trim_passthrough_suffix_operation_create();

  operation_class_t x = t->getType();
  EXPECT_EQ(x, OPERATION_TRIM_PASSTHROUGH_SUFFIX);

  ubiq_platform_trim_passthrough_suffix_operation_delete(t);
}

TEST(operation_trim_passthrough_suffix, invoke)
{
  ubiq_platform_operation_t * const t = ubiq_platform_trim_passthrough_suffix_operation_create();
...
TEST(operation_trim_passthrough_suffix, simple)
{
  ...
  typedef struct  {
   ...
   data_t data[] = { 
    ...
    {U"abc-123", U"abc-", U"123", 3},
    {U"abc-123", U"abc", U"-123", 4}
  };
  ...
  for (int i = 0; i < sizeof(data) / sizeof(data_t); i++) {
    ...
    res = create_operation_context(data[i].suffix_length, &ctx, &dataset);
    ASSERT_EQ(res, 0);
    ASSERT_NE(ctx, nullptr);
    ASSERT_NE(dataset, nullptr);

    ubiq_platform_operation_t * const t = ubiq_platform_trim_passthrough_suffix_operation_create();
    ubiq_platform_operation_t * const expand = ubiq_platform_expand_passthrough_suffix_operation_create();
    ASSERT_NE(t, nullptr);
...

    {
      ...
      if (data[i].suffix_length > 0) {
        char32_t const * const tmp = ubiq_platform_operation_context_get_data_value(ctx, OPERATION_CONTEXT_SUFFIX);
        EXPECT_EQ(strcmp32(tmp, data[i].suffix), 0);
      }
    ...
    ubiq_platform_expand_passthrough_suffix_operation_delete(expand);
    ubiq_platform_trim_passthrough_suffix_operation_delete(t);
